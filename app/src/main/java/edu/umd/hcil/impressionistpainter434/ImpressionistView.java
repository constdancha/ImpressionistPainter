...
public class ImpressionistView extends View {
    ...
    public boolean onTouchEvent(MotionEvent motionEvent){
        ...
        int imageX = (int)touchX;
        int imageY = (int)touchY;
        ...
        switch (motionEvent.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if(_brushType==BrushType.Circle) {
                    ...
                    float max = Math.max(xVelocity,yVelocity);
                    float radius = _minBrushRadius;
                    if(max>1000){
                        radius = 70;
                    }else if(max>500){
                        radius = 50;
                    ...
                    _offScreenCanvas.drawCircle(imageX, imageY, radius, _paint);
                }
                    ...
                    _offScreenCanvas.drawLine(touchX,touchY+_defaultRadius,touchX,touchY-_defaultRadius,_paint);

                }
                else if(_brushType==BrushType.CircleSplatter){
                    for(int i=0;i<5;i++){
                        int x = (int) (touchX + r.nextGaussian()*_defaultRadius);
                        int y = (int) (touchY + r.nextGaussian()*_defaultRadius);
                    ...
                    for(int i=0;i<5;i++){
                        int x = (int) (touchX + r.nextGaussian()*_defaultRadius);
                        int y = (int) (touchY + r.nextGaussian()*_defaultRadius);
                        int x2 = (int) (touchX + r.nextGaussian()*_defaultRadius);
                        int y2 = (int) (touchY + r.nextGaussian()*_defaultRadius);
                        _offScreenCanvas.drawLine(x, y,x2,y2, _paint);
                    }
                }
                break;
    ...
    public void saveBitmap(){
        ...
        MediaStore.Images.Media.insertImage(getContext().getContentResolver(), _offScreenBitmap, "Image" , "Drawn from app");
    }
